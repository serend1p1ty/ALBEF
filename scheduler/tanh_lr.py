...
class TanhLRScheduler(Scheduler):
    ...
    def __init__(self, optimizer: torch.optim.Optimizer, t_initial: int, lb: float = -6.,
                 ub: float = 4., t_mul: float = 1., lr_min: float = 0., decay_rate: float = 1.,
                 warmup_t=0, warmup_lr_init=0, warmup_prefix=False, cycle_limit=0, t_in_epochs=True,
        ...
        super().__init__(optimizer, param_group_field="lr", noise_range_t=noise_range_t,
                         noise_pct=noise_pct, noise_std=noise_std, noise_seed=noise_seed,
        ...
        if t < self.warmup_t:
            ...
            if self.cycle_limit == 0 or (self.cycle_limit > 0 and i < self.cycle_limit):
                ...
                lrs = [
                    ...
                    (1 - math.tanh(self.lb * (1. - tr) + self.ub * tr)) for lr_max in lr_max_values
                ]
            else:
