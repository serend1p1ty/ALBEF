...
def val(model, data_loader, tokenizer, device, gradcam_mode, block_num):
    ...
    for image, text, ref_ids in metric_logger.log_every(data_loader, print_freq, header):
        ...
        if gradcam_mode == 'itm':
            ...
            with torch.no_grad():
                mask = text_input.attention_mask.view(text_input.attention_mask.size(0), 1, -1, 1,
                                                      1)
                ...
                grads = grads[:, :, :, 1:].clamp(min=0).reshape(image.size(0), 12, -1, 24,
                                                                24) * mask
...
def main(args, config):
    ...
    train_loader, test_loader = create_loader(
        datasets, samplers, batch_size=[config['batch_size'], config['batch_size']],
    ...
    for epoch in range(0, max_epoch):
        ...
        result = val(model_without_ddp, test_loader, tokenizer, device, args.gradcam_mode,
                     args.block_num)

        results = collect_result(result, args.result_dir, 'epoch%d' % epoch, is_json=False,
                                 is_list=True)
