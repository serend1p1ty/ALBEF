...
def train(model, data_loader, optimizer, tokenizer, epoch, warmup_steps, device, scheduler, config):
    ...
    for i, (image, text, idx) in enumerate(metric_logger.log_every(data_loader, print_freq,
                                                                   header)):
        image = image.to(device, non_blocking=True)
        idx = idx.to(device, non_blocking=True)
        text_input = tokenizer(text, padding='longest', max_length=30,
...
def evaluation(model, data_loader, tokenizer, device, config):
    ...
    for i, sims in enumerate(metric_logger.log_every(sims_matrix[start:end], 50, header)):
        ...
        output = model.text_encoder(encoder_embeds=text_feats[topk_idx],
                                    ...
                                    encoder_attention_mask=encoder_att, return_dict=True,
                                    mode='fusion')
    ...
    for i, sims in enumerate(metric_logger.log_every(sims_matrix[start:end], 50, header)):
        ...
        output = model.text_encoder(
            ...
            attention_mask=text_atts[start + i].repeat(config['k_test'],
                                                       1), encoder_hidden_states=encoder_output,
            encoder_attention_mask=encoder_att, return_dict=True, mode='fusion')
        score = model.itm_head(output.last_hidden_state[:, 0, :])[:, 1]
...
def main(args, config):
    ...
    for epoch in range(0, max_epoch):
        ...
        score_val_i2t, score_val_t2i, = evaluation(model_without_ddp, val_loader, tokenizer, device,
                                                   config)
        score_test_i2t, score_test_t2i = evaluation(model_without_ddp, test_loader, tokenizer,
                                                    device, config)
